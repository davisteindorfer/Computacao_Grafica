...
bool Cilindro::ValidacaoPontoLateral(const Ponto &p_int) const {
    Vetor PB = operações::SubtracaoPontos(centro, p_int);
    double PB_u = operações::ProdutoEscalar(PB, normal);

    return 0 <= PB_u && PB_u <= altura;
}

Vetor Cilindro::calcularNormal(Ponto* pi){
    Vetor PImenosCB = operações::SubtracaoPontos(centro, *pi);
    ...
    Vetor PImenosPE = operações::SubtracaoPontos(pe, *pi);
...
Ponto *Cilindro::PrimeiraIntersecao(const Ponto &pP0, const Vetor &pVetor0) {
    ...
    auto vetorNorm = operações::NormalizaVetor(pVetor0);

    // C0P0 eh o P0 - C0
    Vetor C0P0 = operações::SubtracaoPontos(centro, pP0);

    // V eh o ((C0P0) - ((C0P0)* vetor_n)* vetor_n)
    Vetor v = C0P0 - (normal * operações::ProdutoEscalar(C0P0, normal));

    // W eh o (pVetor0 - (pVetor0 * vetor_n)*vetor_n)
    Vetor w = vetorNorm - (normal * operações::ProdutoEscalar(vetorNorm,normal));

    //A = w*w
    double a = operações::ProdutoEscalar(w,w);

    //B = (v*w)
    double b = operações::ProdutoEscalar(v,w);

    //C = (v*v - R²)
    double c = operações::ProdutoEscalar(v,v) - (raio*raio);
    ...
    if(p_int1) {
        if(operações::distanciaEntrePontos(*p_int1, centro) > raio){
            p_int1 = nullptr;
        } else {
            menorDistancia = operações::distanciaEntrePontos(pP0, *p_int1);
        }
    }

    // Intersecao com o topo
    Ponto* p_int2 = baseSup.IntersecaoRetaPlano(pP0,pVetor0);
    if(p_int2 && operações::distanciaEntrePontos(*p_int2, centroSup) <= raio) {
        double dist2 = operações::distanciaEntrePontos(*p_int2, pP0);
    ...
    Ponto* p_teste1 = operações::EquacaoDaReta(pP0, t_int1, vetorNorm);
    Ponto* p_teste2 = operações::EquacaoDaReta(pP0, t_int2, vetorNorm);


    if (ValidacaoPontoLateral(*p_teste1)){
        double dist1 = operações::distanciaEntrePontos(*p_teste1, pP0);
    ...
    if (ValidacaoPontoLateral(*p_teste2)) {
        double dist2 = operações::distanciaEntrePontos(*p_teste2, pP0);
...
tuple<Ponto, Ponto> Cilindro::Limites()
{
    ...
    Vetor prod(1, 0, 0);
    if(normal.y != 1) {
        prod = operações::ProdutoVetorial(normal, Vetor(0, 1, 0));
    }
    Vetor prod2 = operações::ProdutoVetorial(prod, normal);
    for(int i = 0; i < 4; i ++) {
        ...
        prod = prod2;
        prod2 = operações::ProdutoVetorial(prod, normal);
    }
